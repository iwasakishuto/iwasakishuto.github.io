...
window.RevealSlickQuiz = window.RevealSlickQuiz || {
	id: 'RevealSlickQuiz',
	init: function(deck) {
		initSlickQuiz(deck);
	},
...
const initSlickQuiz = function(Reveal){
  ...
  for (var i = 0, len = quizzes.length; i < len; i++) {
    var quizNode = quizzes[i];
    quizNode.className = "slickQuiz"
    var quizSource = quizNode.innerHTML.trim().replace(/<!--/,'').replace(/-->/,'');
    // Remove Comments.
    var classname = 'slickQuiz-' + i;
    createQuizStructure(quizNode, classname);

    // Define the quiz. (quiz = {...})
    eval("quiz = " + quizSource)
    options.json = quiz;
  ...
  <div id="` + classname + `">
          ...
          <div class="quizArea">
              <div class="quizHeader">
                  ...
                  <a class="button startQuiz" href="#" style="display: inline;">Get Started!</a>
              </div>
