...
const initChalkboard = function(Reveal){
	...
	******************************************************************/
	...
	var help_icon_color = "#00ffa9";
	var grid = false;
	var boardmarkerWidth = 3;
	var chalkWidth = 7;
	var chalkEffect = 1.0;
	var rememberColor = [true, false];
	var eraser = { src: path + 'chalkboard-img/sponge.png', radius: 20};
	var boardmarkers = [
		{ color: 'rgba(255, 255, 255, 1)', cursor: 'url(' + path + 'chalkboard-img/boardmarker-white.png),  auto'},
		{ color: 'rgba( 30, 144, 255, 1)', cursor: 'url(' + path + 'chalkboard-img/boardmarker-blue.png),   auto'},
		{ color: 'rgba(220,  20,  60, 1)', cursor: 'url(' + path + 'chalkboard-img/boardmarker-red.png),    auto'},
		{ color: 'rgba( 50, 205,  50, 1)', cursor: 'url(' + path + 'chalkboard-img/boardmarker-green.png),  auto'},
		{ color: 'rgba(255, 140,   0, 1)', cursor: 'url(' + path + 'chalkboard-img/boardmarker-orange.png), auto'},
		{ color: 'rgba(150,   0, 150, 1)', cursor: 'url(' + path + 'chalkboard-img/boardmarker-purple.png), auto'},
		{ color: 'rgba(255, 220,   0, 1)', cursor: 'url(' + path + 'chalkboard-img/boardmarker-yellow.png), auto'},
		{ color: 'rgba(100, 100, 100, 1)', cursor: 'url(' + path + 'chalkboard-img/boardmarker-black.png),  auto'},
	];
	var chalks = [
		{ color: 'rgba(255, 255, 255, 0.5)', cursor: 'url(' + path + 'chalkboard-img/chalk-white.png),  auto'},
		{ color: 'rgba( 96, 154, 244, 0.5)', cursor: 'url(' + path + 'chalkboard-img/chalk-blue.png),   auto'},
		{ color: 'rgba(237,  20,  28, 0.5)', cursor: 'url(' + path + 'chalkboard-img/chalk-red.png),    auto'},
		{ color: 'rgba( 20, 237,  28, 0.5)', cursor: 'url(' + path + 'chalkboard-img/chalk-green.png),  auto'},
		{ color: 'rgba(220, 133,  41, 0.5)', cursor: 'url(' + path + 'chalkboard-img/chalk-orange.png), auto'},
		{ color: 'rgba(220,   0, 220, 0.5)', cursor: 'url(' + path + 'chalkboard-img/chalk-purple.png), auto'},
		{ color: 'rgba(255, 220,   0, 0.5)', cursor: 'url(' + path + 'chalkboard-img/chalk-yellow.png), auto'},
	];
	...
	var keyBindings = { 
		toggleNotesCanvas: { keyCode: 67,  key: 'C',         description: '<i style="color: ' + help_icon_color + ';" class="fa fa-pen"></i> Toggle notes canvas' },
		toggleChalkboard:  { keyCode: 66,  key: 'B',         description: '<i style="color: ' + help_icon_color + ';" class="fa fa-pen-square"></i> Toggle chalkboard' },
		clear:             { keyCode: 171, key: '+',         description: '<i style="color: ' + help_icon_color + ';" class="fas fa-eraser"></i> Clear drawings on slide' },
		reset:             { keyCode: 46,  key: 'DEL',       description: '<i style="color: ' + help_icon_color + ';" class="fas fa-eraser"></i> Reset drawings on slide' },
		resetAll:          { keyCode: 8,   key: 'BACKSPACE', description: '<i style="color: ' + help_icon_color + ';" class="fas fa-redo"></i> Reset all drawings' },
		colorNext:         { keyCode: 88,  key: 'X',         description: '<i style="color: ' + help_icon_color + ';" class="fas fa-palette"></i> Next color' },
		colorPrev:         { keyCode: 89,  key: 'Y',         description: '<i style="color: ' + help_icon_color + ';" class="fas fa-palette"></i> Previous color' },
		download:          { keyCode: 68,  key: 'D',         description: '<i style="color: ' + help_icon_color + ';" class="fas fa-download"></i> Download drawings' }
	};
	...
	function configure( config ) {
		...
		switch ( theme ) {
			case "whiteboard":
				background = [ 'rgba(127,127,127,.1)' , path + 'chalkboard-img/whiteboard.png' ];
			...
			case "chalkboard":
			default:
				background = [ 'rgba(127,127,127,.1)' , path + 'chalkboard-img/blackboard.png' ];
		...
		if ( config.help_icon_color) help_icon_color = config.help_icon_color
	...
	******************************************************************/
	...
	function drawCurve( id, event, timestamp ) {
		if  ( event.curve.length > 1 ) {
			...
			console.log("drawCurve", drawingCanvas[id])
	...
	*/
	...
	/*
			...
			if ( evt.button == 2) {
				...
				drawingCanvas[mode].canvas.style.cursor = 'url("' + path + 'chalkboard-img/sponge.png") ' + eraser.radius + ' ' + eraser.radius + ', auto';
				eraseWithSponge(ctx,mouseX,mouseY);
			}
			else {
				event = { type: "draw", begin: Date.now() - slideStart, end: null, curve: [{x: (mouseX - xOffset)/scale, y: (mouseY-yOffset)/scale}] };
	...
	*/
	...
	//			var ctx = drawingCanvas[mode].context;
			var scale = drawingCanvas[mode].scale;
			var xOffset = drawingCanvas[mode].xOffset; // NaN
			var yOffset = drawingCanvas[mode].yOffset; // NaN
